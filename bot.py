#!/usr/bin/env python3
"""
Discord VPS Bot (Nextcord) - bot.py

Features:
- Invite tracking: 1 invite = 1 point
- Owner can add VPS plans via modal (/plan-add)
- Users can view plans (/plans)
- Users can claim VPS (/claim-vps) via select + modal
- Manage VPSes (/manage) with buttons (Start/Stop/Restart/Stats/SSH)
- /points, /leaderboard
- Status rotation every 5 seconds
- Data stored in local JSON files (users.json, plans.json, vps.json, admins.json, invites.json)
- LXD/pylxd integration for actual container creation (if available)
- Suspend/delete logic when user points drop below requirement
- Admin system (admins.json)
- Embeds include BOT_LOGO_URL thumbnail and footer "Made by GG!"

Note: This is a ready-to-run single-file bot. Make sure .env is populated and LXD is configured if you want pylxd container creation.
"""
...
bot = commands.Bot(intents=intents, command_prefix="!", sync_commands=True)
...
class LXDManager:
    ...
    def create_container(self, name: str, image_alias: str, cpu: int, ram_gb: int, disk_gb: int, root_password: str) -> Dict[str, Any]:
        """
        Create LXD container with resource limits. Return dict with success and container_name.
        Note: This expects an image with alias image_alias to exist. For cloud-init root password we will try to run chpasswd
        after the container starts. This code can fail depending on LXD configuration and available images.
        """
        if not self.available():
            return {"success": False, "error": "pylxd not available or LXD not accessible"}

        # Simplified resource config
        config = {
            "name": name,
...
class ClaimSelectView(View):
    def __init__(self, user: nextcord.User, timeout: Optional[float] = 120):
        ...
        # will populate selects dynamically
        # add placeholders; actual options added when view is instantiated in command
    ...
    async def plan_select(self, select: Select, interaction: Interaction):
        ...
        # If OS already selected then prompt for password
        if self.selected_os:
...
class ManageView(View):
    ...
    @Button(style=nextcord.ButtonStyle.green, label="Start", custom_id="btn_start")
    ...
    @Button(style=nextcord.ButtonStyle.danger, label="Stop", custom_id="btn_stop")
    ...
    @Button(style=nextcord.ButtonStyle.secondary, label="Restart", custom_id="btn_restart")
    ...
    @Button(style=nextcord.ButtonStyle.blurple, label="Stats", custom_id="btn_stats")
    ...
    @Button(style=nextcord.ButtonStyle.url, label="SSH (tmate)", url="https://tmate.io", custom_id="btn_ssh")
    async def ssh_button(self, button: Button, interaction: Interaction):
        # Button is a URL; custom_id not used.
        await interaction.response.send_message("Open the SSH (tmate) link provided in the embed.", ephemeral=True)
...
async def suspend_monitor():
    ...
    for vps_id, v in list(vps_all.items()):
        ...
        if owner_points < required and not suspended:
            ...
            # check if suspended long enough to delete
            if suspended_at:
...
async def on_guild_join(guild):
    # populate invites for new guild
    try:
        invites = await guild.invites()
        invites_cache[guild.id] = {inv.code: inv.uses for inv in invites}
    except Exception:
...
async def on_member_join(member):
    ...
    if used_inviter:
        ...
        # record invites in invites.json
...
async def claim_vps(interaction: Interaction):
    ...
    # Replace first select with our dynamic options by manipulating children
    # Set options on plan_select
    for child in view.children:
        if isinstance(child, nextcord.ui.Select) and child.custom_id == "claim_plan_select":
...
async def manage_cmd(interaction: Interaction):
    ...
    if not user_vps:
        ...
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    # Build select of user's VPS ids
    options = []
    for v in user_vps:
        vid = v.get("vps_id")
    ...
    class VPSSelectView(View):
        ...
        async def select_callback(self, select: Select, interaction2: Interaction):
            ...
            # Set the SSH button URL dynamically by replacing the URL button (since nextcord requires url at creation)
            # We'll create a new URL button manually and add to view
            # Remove any existing url button placeholder
            # Add URL button
...
async def leaderboard_cmd(interaction: Interaction):
    ...
    if not users:
        ...
        return
    # users is dict keyed by user id with 'points'
    arr = []
...
class AdminCreateVPSModal(Modal):
    ...
    async def callback(self, interaction: Interaction):
        # only admins allowed (checked before showing modal, but double-check)
        ...
        plan_id = await add_plan(plan_data)
        # deduct points from target? For admin-created VPS we won't deduct; but record vps entry
        vps_id = generate_vps_id()
        container_name = f"vps-{vps_id}"
        ...
        await save_vps(vps_id, vps_entry)
        # Try to create container if lxd available
        if lxd.available():
...
async def admin_create_vps(interaction: Interaction, target: nextcord.User = SlashOption(description="User to create VPS for")):
    if not await is_admin(interaction.user.id):
        await interaction.response.send_message("You are not an admin.", ephemeral=True)
        return
    # show modal to collect plan specs
